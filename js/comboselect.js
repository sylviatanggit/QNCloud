...
;(function($){
	...
	$.extend(ComboSelect.prototype,{
		...
		_searchForJson: function(self, q_word, which_page_num) {
			...
			for (i = 0; i < self.option.source.length; i++) {
				...
				for (var j = 0; j < arr_reg.length; j++) {					
					...
					if(typeof(row.companysid) != 'undefined' && row.companysid !="" && row.companysid){
						itemText = itemText + row.companysid.toUpperCase();
					}
		...
		_displayResults: function(self, json, is_query) {
			...
			var originalResult = json.originalResult;
			...
			for (var i = 0; i < arr_candidate.length; i++) {
				...
				if(typeof(originalResult[i].producecount) !="undefined" && originalResult[i].producecount == 0){
					$(list).addClass(self.css_class.red_style);
				}
